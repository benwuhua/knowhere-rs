...
- 🔄 `HNSW-REOPEN-001`: round 3 is now active.
  ...
  - The current active question is no longer whether candidate-search broadly matters; it is whether the remaining `distance_compute` hotspot can be split into actionable sub-paths and then improved without another authority regression.
  - The next tracked feature is `hnsw-distance-compute-profiler`.
...
3. Round 3 现已用新的 authority-backed hypothesis 重开：当前 active line 只允许围绕 `distance_compute_inner_loop` 推进。只有当新的 round-3 authority artifact 真正改善 same-schema lane，才允许重开 family-level leadership 或 project-level acceptance 叙事。
