...
## 轮次记录
- 2026-03-14: **builder-loop：完成 `hnsw-round12-authority-same-schema-rerun`，Rust authority lane 成功跑完但以 hard stop 收口（authority/hard_stop）**
  1. 复核输入：`task-progress.md`、`feature-list.json`、`tests/bench_hnsw_reopen_round12.rs`、`benchmark_results/hnsw_reopen_round12_baseline.json`、`benchmark_results/hnsw_reopen_round11_authority_summary.json`、`docs/superpowers/plans/2026-03-14-hnsw-round12-authority-same-schema-rerun.md`。
  2. 阶段结论：上一轮已经把问题缩到 authority 代理链路，而不是 HNSW 代码本身。本轮先确认 SSH/Proxy 已恢复，并发现之前被中断的 isolated round-12 Rust same-schema rerun 实际上仍在远端继续执行；因此这次不需要重跑 Rust benchmark，只需要监控它跑完、补齐 native capture、同步 artifact、再用真实 authority evidence 做最终结论。
  3. 本轮执行：监控 `/data/work/knowhere-rs-logs-hnsw-reopen-round12/test_20260314T060359Z_20504.status` 直到 `status=ok`，随后把 fresh `benchmark_results/rs_hnsw_sift128.full_k100.json` 拉回本地，拿到 Rust `1497.689` qps / recall `0.9942`；再通过 linkfix fallback 运行 native benchmark，得到 `/data/work/knowhere-rs-logs-hnsw-reopen-round12/native_hnsw_qps_linkfix_20260314T062053Z.log` 中的 trusted native row `8502.128` qps / recall `0.95`。最后重新收紧 round-12 default-lane contract，新增 `benchmark_results/hnsw_reopen_round12_authority_summary.json`，并回放 remote default-lane replay。
  4. 验证结果：round-12 Rust authority lane 这次确实完成了，不再是 round-11 那样的灾难性 abort；但结果依然是负面的 authority evidence。相比 round 10，Rust qps 从 `1831.504` 回落到 `1497.689`（约 `-18.2%`），native 也从 `8900.882` 降到 `8502.128`（约 `-4.5%`），导致 native-over-Rust gap 反而从 `4.86x` 扩大到 `5.68x`。这说明 Task-16 的 local `shared_bitset_batch4` screen win 没能迁移成 remote same-schema gain。
  5. 后续主缺口：`shared_bitset_batch4` 应以 `hard_stop` 归档，历史 HNSW family verdict 继续保持不变。当前 `feature-list.json` 已恢复到 `66/66` 全绿；如果未来还要重开 HNSW，必须从新的 local `screen_result=promote` 重新起线，而不是继续沿用 round-12 这条 authority 已否定的假设。

- 2026-03-14: **builder-loop：尝试执行 `hnsw-round12-authority-same-schema-rerun`，但 authority 代理链路在 benchmark 过程中反复 reset（authority/blocked）**
  1. 复核输入：`task-progress.md`、`feature-list.json`、`tests/bench_hnsw_reopen_round12.rs`、`benchmark_results/hnsw_reopen_round12_baseline.json`、`docs/superpowers/plans/2026-03-14-hnsw-round12-authority-same-schema-rerun.md`、`benchmark_results/hnsw_reopen_round11_authority_summary.json`。
  2. 阶段结论：round-12 activation 已经完成，当前唯一缺口确实是 authority same-schema rerun。按 round-10/11 的模式，这一轮先把 round-12 default-lane contract 临时收紧到必须要求 authority summary artifact，并确认本地因缺失 `benchmark_results/hnsw_reopen_round12_authority_summary.json` 而转红；之后就只剩 authority benchmark、native capture、artifact sync-back、summary 收口四步。
  3. 本轮执行：`bash init.sh` 成功，说明 authority bootstrap 在起跑时是正常的；随后把隔离的 round-12 Rust same-schema benchmark 发到 `/data/work/knowhere-rs-target-hnsw-reopen-round12` / `/data/work/knowhere-rs-logs-hnsw-reopen-round12`。但在等待远端结果时，SSH/proxy 控制链路被 `socks5 proxy error: [Errno 54] Connection reset by peer` 直接切断；紧接着的极小 `run_ssh "echo remote_ok"` 探针也复现了同样的 reset。
  4. 验证结果：当前拿到的是环境级阻塞证据，而不是 benchmark verdict。也就是说，这次没有得到可信的 round-12 Rust qps、没有 fresh native capture、也没有新的 authority summary artifact 可归档。为了不把仓库留在默认红灯状态，本轮已把临时收紧的 round-12 authority-summary contract 回退回 activation 态。
  5. 后续主缺口：下一步不是修改 HNSW 代码，而是先等待或恢复 authority 代理链路。远端连通性恢复后，应先用一个 trivial `run_ssh` 探针确认健康，再重新收紧 round-12 contract、重跑 isolated same-schema Rust benchmark、fresh native capture、artifact sync-back，并据此决定 `shared_bitset_batch4` 在 authority lane 上是 `continue`、`soft_stop` 还是 `hard_stop`。

- 2026-03-14: **builder-loop：补跑并关闭 `hnsw-reopen-round12-activation`，authority bootstrap 与 remote default-lane replay 均通过（activation/passing）**
  1. 复核输入：`task-progress.md`、`feature-list.json`、`tests/bench_hnsw_reopen_round12.rs`、`benchmark_results/hnsw_reopen_round12_baseline.json`、`docs/superpowers/plans/2026-03-14-hnsw-reopen-round12-activation.md`。
  2. 阶段结论：上一轮缺的不是代码，也不是 activation contract 本身，而是 authority-required bootstrap/replay 证据。当前环境恢复远端访问后，这一轮的最小诚实动作就是补齐 `bash init.sh` 和 round-12 remote default-lane replay，把 round-12 activation 从“准备完成但未验证”推进到真正 `passing`。
  3. 本轮执行：重新运行 `bash init.sh`，确认 workspace 已同步到 remote x86 且 toolchain probe 正常；随后使用隔离的 `KNOWHERE_RS_REMOTE_TARGET_DIR=/data/work/knowhere-rs-target-hnsw-reopen-round12` 与 `KNOWHERE_RS_REMOTE_LOG_DIR=/data/work/knowhere-rs-logs-hnsw-reopen-round12` 回放 `cargo test --test bench_hnsw_reopen_round12 -q`，验证新的 round-12 baseline artifact 与被有意忽略的 future authority-summary contract 一并在 authority 路径上存活。
  4. 验证结果：本地 `cargo test --test bench_hnsw_reopen_round12 -- --nocapture` 继续通过；`bash init.sh` 成功完成远端同步并打印 authority ready；remote default-lane replay 返回 `test=ok`，日志为 `/data/work/knowhere-rs-logs-hnsw-reopen-round12/test_20260314T055626Z_16735.log`；因此 `hnsw-reopen-round12-activation` 现在可以诚实地标成 `passing`。
  5. 后续主缺口：当前唯一未关闭的 tracked feature 已切到 `hnsw-round12-authority-same-schema-rerun`。下一步必须在同一组 round-12 隔离目录上运行 recall-gated Rust same-schema benchmark、native capture 与 round-12 contract replay，回答 Task-16 `shared_bitset_batch4` 是否能在 authority lane 上产出任何新的 stop-go 证据。

- 2026-03-14: **builder-loop：重开 `hnsw-reopen-round12-activation`，把 promoted Task-16 local screen 准备成 tracked authority baseline，但被沙箱拦在 remote bootstrap（activation/blocked）**
  1. 复核输入：`task-progress.md`、`docs/superpowers/plans/2026-03-14-hnsw-reopen-round12-activation.md`、`benchmark_results/hnsw_reopen_round11_authority_summary.json`、`benchmark_results/hnsw_bitset_search_cost_diagnosis.json`、`feature-list.json`、`tests/bench_hnsw_reopen_round11.rs`。
  2. 阶段结论：Task 16 已经给出足够强的 `screen_result=promote`，因此下一步不该继续停留在 local screen，而应按 round11 的模式把它转成 tracked authority-grade work。这个 activation slice 只需要做三件事：新增 round-12 contract、冻结 baseline artifact、重开 durable feature state；authority same-schema rerun 留到下一轮。
  3. 本轮执行：新增 `tests/bench_hnsw_reopen_round12.rs`，先用缺失 `benchmark_results/hnsw_reopen_round12_baseline.json` 的失败做 TDD red；随后创建该 baseline artifact，把 round-11 authority hard stop 和 Task-16 promoted bitset-lane evidence 冻结成 round-12 起点，并在 `feature-list.json` 中新增 `hnsw-reopen-round12-activation` 与 `hnsw-round12-authority-same-schema-rerun` 两个 tracked feature。新的 activation baseline 明确把 round12 target 锁成 `shared_bitset_batch4`。
  4. 验证结果：本地 `cargo test --test bench_hnsw_reopen_round12 -- --nocapture` 先因缺失 baseline artifact 失败，随后通过，且 authority-summary check 被有意 `#[ignore]` 到下一 slice；但 `bash init.sh` 在 authority bootstrap 的 ssh/proxy 阶段被当前沙箱直接拦截，报 `socks5 proxy error: [Errno 1] Operation not permitted` 并断开连接，因此 round-12 activation 所需的 remote default-lane replay 没有跑成。
  5. 后续主缺口：当前不能把 `hnsw-reopen-round12-activation` 标成 `passing`，因为 authority-required verification 没完成。下一步必须先在允许远端访问的环境里重跑 `bash init.sh` 和 round-12 remote default-lane replay；只有 activation 真正通过后，才应该进入 `hnsw-round12-authority-same-schema-rerun`。

- 2026-03-14: **builder-loop：执行 `hnsw-search-first-task16` 本地 screen slice，把 shared bitset layer-0 L2 距离切到 grouped batch-4，并以 promote 收口（screen/promote）**
  1. 复核输入：`task-progress.md`、`docs/superpowers/specs/2026-03-14-hnsw-search-first-task16-filtered-batch4-design.md`、`docs/superpowers/plans/2026-03-14-hnsw-search-first-task16.md`、`/tmp/hnsw_bitset_search_cost_diagnosis.task14.baseline.json`、`tests/bench_hnsw_generic_search_kernel.rs`、`src/faiss/hnsw.rs`。
  2. 阶段结论：Task 15 已经给出 `result container swap` 的明确负信号，因此 `search_first` 不能再继续围着结果容器打磨。下一刀必须换机制，但仍要留在 deterministic bitset lane 上。最合适的切点，是把 shared generic/bitset kernel 在 `level=0 + L2 + flat_graph` 上的逐邻居标量距离计算，改成只对有效未过滤邻居做 grouped batch-4。
  3. 本轮执行：先在 `tests/bench_hnsw_generic_search_kernel.rs` 中新增 `shared_bitset_distance_mode=idx_ptr_batch4_when_grouped` 的 contract 做 TDD red；随后在 `src/faiss/hnsw.rs` 中扩展 `HnswGenericSearchKernelReport`，加入 deterministic `deterministic_layer0_batch4_index()` fixture 与 `test_search_layer_idx_shared_uses_batch4_distance_when_grouping_layer0_neighbors`，再把 `search_layer_idx_shared(...)` 的 `level=0 + L2 + flat_graph + bitset` 分支改成累计有效邻居到 4 个一组并复用 `l2_distance_to_4_idxs_ptr(...)`，尾部不足 4 个的邻居继续走原标量 helper，accepted-neighbor frontier/result 逻辑保持不变。
  4. 验证结果：contract 与 focused regressions 最终都通过；canonical deterministic bitset diagnosis lane 上，recall 仍固定 `0.9942`，visited / distance / frontier counters 与 Task-14 baseline 完全一致，而 qps 相比 Task-14 baseline 分别提升约 `+15.68%`、`+17.57%`、`+16.50%`、`+17.81%`（`ef=64/96/128/160`）；同代码 `/tmp` rerun 的 drift 仅约 `+0.84%`、`+1.83%`、`+0.60%`、`-0.58%`。这说明 grouped shared bitset batch-4 不是噪声，而是在当前 lane 上给出了另一条清晰的正向信号。附带的 unfiltered diagnosis artifact 只在 `-0.97% .. +3.78%` 的更小区间内波动且 counters 保持不变，这与本轮优化刻意只打 bitset/shared path 的边界一致。
  5. 后续主缺口：这轮应记为 `screen_result=promote`，但它依然只是 local screen evidence，不构成 authority 结论。后续如果继续 pure-Rust HNSW `search_first`，应优先把这一 grouped shared bitset batch-4 机制带到 authority same-schema lane，而不是马上再扩成更大的 generic-kernel rewrite。

- 2026-03-14: **builder-loop：执行 `hnsw-search-first-task15` 本地 screen slice，测试 shared generic/bitset result-container swap，并以 reject 收口（screen/reject）**
  1. 复核输入：`task-progress.md`、`docs/superpowers/specs/2026-03-14-hnsw-search-first-task15-result-container-swap-design.md`、`docs/superpowers/plans/2026-03-14-hnsw-search-first-task15.md`、`benchmark_results/hnsw_bitset_search_cost_diagnosis.json`、`tests/bench_hnsw_generic_search_kernel.rs`、`src/faiss/hnsw.rs`。
  2. 阶段结论：Task 14 已经说明继续围绕 worst-threshold 做小修补不会再给出 promotable signal，因此 Task 15 选择了一个更大的 shared-kernel 结构切片：保留 `generic_frontier` heap，只把 shared generic/bitset bounded results 从 `BinaryHeap` 换成 scratch-owned sorted vec。
  3. 本轮执行：先在 `tests/bench_hnsw_generic_search_kernel.rs` 中新增 `shared_result_container_mode=sorted_scratch_vec` 的 contract 做 TDD red；随后在 `src/faiss/hnsw.rs` 中临时扩展 `HnswGenericSearchKernelReport`，加入 focused regression `test_search_layer_idx_shared_sorted_results_stay_nearest_first_across_calls`，并实现 shared-kernel result-container swap。第一次 local bitset rerun 暴露出 visited/distance/frontier counters 轻微漂移，因此先把 sorted vec 的 tie 顺序修正到和原 heap 一致的 `(dist, idx)` 语义，再次重跑诊断 lane。
  4. 验证结果：contract 与 focused regressions 最终都通过；tie-order 修正后，recall 仍固定 `0.9942`，visited / distance / frontier counters 与 Task-14 baseline 重新完全一致，但 canonical bitset diagnosis lane 的 qps 相比 Task-14 baseline 仍分别回落约 `-1.60%`、`-2.85%`、`-2.81%`、`-2.44%`（`ef=64/96/128/160`）；同代码 `/tmp` rerun 的 drift 约为 `-1.62%`、`+1.33%`、`+0.56%`、`-0.44%`。这意味着 shared result-container swap 不是“没看见收益”，而是在当前 deterministic bitset lane 上给出了明确的负信号。确认结论后，本轮把 Task-15 代码回退到 Task-14 状态，并重新生成 canonical local artifacts，避免仓库工作区停在一个已拒绝的实现上。
  5. 后续主缺口：这轮应该记为 `screen_result=reject`。后续如果继续 `search_first`，不应再沿 `sorted result container` 这个机制继续局部打磨，而应该换一个不同的 shared-kernel 假设；否则只会在一个已经给出负收益的结构方向上继续消耗时间。

- 2026-03-14: **builder-loop：执行 `hnsw-search-first-task14` 本地实现 slice，把 shared generic/bitset kernel 的最差结果阈值缓存进 `SearchScratch`（screen/impl）**
  1. 复核输入：`task-progress.md`、`docs/superpowers/specs/2026-03-14-hnsw-search-first-task14-frontier-result-reuse-design.md`、`docs/superpowers/plans/2026-03-14-hnsw-search-first-task14.md`、`benchmark_results/hnsw_bitset_search_cost_diagnosis.json`、`tests/bench_hnsw_generic_search_kernel.rs`、`src/faiss/hnsw.rs`。
  2. 阶段结论：Task 13 已经证明 shared generic/bitset kernel 在 deterministic bitset lane 上确实还能出可观测 gain，因此下一刀不应该回去改 harness，也不应该直接扩成更大的搜索核重写。最小可信的一刀，是把 shared kernel 里反复从 `generic_results.peek()` 恢复最差结果阈值的成本收进 `SearchScratch`，看这部分 `frontier/result` 管理开销是否还能被同一条 lane 看到。
  3. 本轮执行：先在 `tests/bench_hnsw_generic_search_kernel.rs` 中新增 `shared_result_threshold_mode=scratch_cached_worst` 的 contract 做 TDD red；随后在 `src/faiss/hnsw.rs` 中扩展 `HnswGenericSearchKernelReport`，为 `SearchScratch` 增加 `generic_worst_result_distance`，在 `prepare_generic_heaps(...)` 中重置它，并仅在 `generic_results` 真正变化时同步；最后把 `search_layer_idx_shared(...)` 的 early-stop 与 accepted-neighbor threshold check 改成读取这条 scratch cache，同时保留结果堆作为最终输出排序的 source of truth。
  4. 验证结果：`cargo test --test bench_hnsw_generic_search_kernel -- --nocapture` 先因缺字段失败后转绿；新增 `test_search_layer_idx_shared_cached_worst_threshold_resets_across_calls` 先因缺失 `generic_worst_result_distance` 失败后转绿；existing shared-layer behavior locks 继续通过；canonical bitset diagnosis lane 重跑后，recall 仍固定 `0.9942`，visited / distance / frontier counters 与 Task 13 完全一致，而 qps 相比 Task-13 baseline 仅变化约 `+0.31%`、`-0.11%`、`+0.16%`、`+0.84%`（`ef=64/96/128/160`）；同代码 `/tmp` rerun 中，same-code drift 约为 `-0.30%`、`+0.78%`、`+0.63%`、`-0.50%`。完成代码后，`cargo fmt --all -- --check` 先抓到一处新的折行差异，`cargo fmt --all` 后再次通过；最终 fresh `bench_hnsw_generic_search_kernel`、`bench_hnsw_search_first_bitset_diagnosis`、`cargo test hnsw --lib -- --nocapture` 和 `python3 scripts/validate_features.py feature-list.json` 全部转绿。
  5. 后续主缺口：这轮说明 cached worst-threshold 是正确方向上的 shared-kernel 收敛，但在当前 deterministic bitset lane 上仍属于 sub-1% 级信号，因此只能记为 `screen_result=needs_more_local`。下一步应继续盯 shared generic/bitset kernel 中更大颗粒的 accepted-neighbor frontier/result mutation 成本，而不是把这条 sub-1% micro-slice 过早 promote 成 authority reopen。

- 2026-03-14: **builder-loop：执行 `hnsw-search-first-task13` 本地实现 slice，把 shared generic/bitset kernel 的 layer-0 L2 邻接读取切到 `layer0_flat_graph`（screen/promote）**
  1. 复核输入：`task-progress.md`、`docs/superpowers/plans/2026-03-14-hnsw-search-first-task13.md`、`benchmark_results/hnsw_bitset_search_cost_diagnosis.json`、`tests/bench_hnsw_generic_search_kernel.rs`、`src/faiss/hnsw.rs`。
  2. 阶段结论：Task 12 已经说明只动 L2 distance dispatch 的收益太接近噪声带；下一刀必须打到 shared generic/bitset kernel 在 layer 0 上更重的邻接访问成本。最小且可测的一刀，是让 `search_layer_idx_shared(...)` 在 `level=0 + L2 + flat_graph enabled` 时直接读 `layer0_flat_graph.neighbors_for(...)`，绕过 canonical `layer_neighbors[level].ids` 和逐邻居 `id -> idx` 转换。
  3. 本轮执行：先在 `tests/bench_hnsw_generic_search_kernel.rs` 中新增 `shared_layer0_neighbor_layout=flat_u32_adjacency_when_enabled` 的 contract 做 TDD red；随后在 `src/faiss/hnsw.rs` 中扩展 `HnswGenericSearchKernelReport`，并把 `search_layer_idx_shared(...)` 的 layer-0 L2 邻接遍历切到 `layer0_flat_graph`，同时保留 non-L2 / non-flat-graph 的 canonical fallback，确保 bitset 过滤语义、frontier/result 顺序和 profile 计数都不变。
  4. 验证结果：`cargo test --test bench_hnsw_generic_search_kernel -- --nocapture` 先因缺字段失败后转绿；existing shared-layer behavior locks 继续通过；canonical bitset diagnosis lane 重跑后，recall 仍固定 `0.9942`，visited / distance-calls 与 Task 12 完全一致，而 qps 相比 Task-12 baseline 分别提升约 `+15.21%`、`+14.86%`、`+13.11%`、`+13.58%`（`ef=64/96/128/160`）；同代码 `/tmp` rerun 中，same-code drift 仅约 `-0.20%`、`-0.67%`、`-0.82%`、`+0.15%`。完成代码后，`cargo fmt --all -- --check` 先抓到一处新的折行差异，`cargo fmt --all` 后再次通过；最终 fresh `bench_hnsw_generic_search_kernel`、`bench_hnsw_search_first_bitset_diagnosis`、`cargo test hnsw --lib -- --nocapture` 和 `python3 scripts/validate_features.py feature-list.json` 全部转绿。
  5. 后续主缺口：这轮已经给出第一条明显超出本地 bitset lane 噪声带的 shared-kernel gain，因此可以记为 `screen_result=promote`。但这仍然只是 local screen evidence，不足以改写 HNSW authority verdict。下一步应继续沿 shared generic/bitset kernel 深挖，优先选取另一个能在同一 deterministic bitset lane 上被直接观测到的 layer-0 邻接或结果管理成本，而不是重新回到 harness 改造。

- 2026-03-14: **builder-loop：执行 `hnsw-search-first-task12` 本地实现 slice，把 shared generic/bitset kernel 的 L2 distance 改成 ptr-specialized dispatch（screen/impl）**
  1. 复核输入：`task-progress.md`、`docs/superpowers/plans/2026-03-14-hnsw-search-first-task12.md`、`benchmark_results/hnsw_bitset_search_cost_diagnosis.json`、`tests/bench_hnsw_generic_search_kernel.rs`、`src/faiss/hnsw.rs`。
  2. 阶段结论：Task 11 已经把 generic/bitset path 的 local measurement spine 补齐，所以这次可以开始做真正的 kernel 改动了。最小、最可信的一刀是让 `search_layer_idx_shared(...)` 在 L2 场景下也走已有的 ptr-specialized distance helper，而不是继续吃 `self.distance(...)` 的通用分发成本。
  3. 本轮执行：先在 `tests/bench_hnsw_generic_search_kernel.rs` 中新增 `shared_l2_distance_dispatch=idx_ptr_kernel` 的 contract 做 TDD red；随后在 `src/faiss/hnsw.rs` 中扩展 `HnswGenericSearchKernelReport`，并把 `search_layer_idx_shared(...)` 的 entry / neighbor distance 在 `MetricType::L2` 下改成通过 `l2_distance_to_idx_ptr(...)` 计算，非 L2 path 则保留原来的 generic dispatch。
  4. 验证结果：`cargo test --test bench_hnsw_generic_search_kernel -- --nocapture` 先因缺字段失败后转绿；existing shared-kernel behavior locks 继续通过；canonical bitset diagnosis lane 重跑后，recall / visited / distance-calls 保持完全不变，而 qps 相比 Task-11 baseline 约提升 `+0.19%`、`+1.28%`、`+1.81%`、`+0.79%`（`ef=64/96/128/160`）；同代码 `/tmp` rerun 中，same-code drift 约为 `+0.14%`、`+1.36%`、`+0.50%`、`+0.44%`，说明当前 signal 虽然偏正，但还没有明显大到脱离这条 lane 自己的噪声带。
  5. 后续主缺口：这意味着 shared-kernel L2 specialization 很可能是对的方向，但单独这一刀的收益还不够硬。下一步如果继续 `search_first`，应该挑一个比 distance dispatch 更高杠杆的 generic/bitset bottleneck，或者先累积更多 local rerun 再决定是否值得 promote。

- 2026-03-14: **builder-loop：执行 `hnsw-search-first-task11` 本地 screen slice，补一条 dedicated bitset/generic diagnosis lane（screen/promote）**
  1. 复核输入：`task-progress.md`、`docs/superpowers/plans/2026-03-14-hnsw-search-first-task11.md`、`src/faiss/hnsw.rs`、`src/bin/generate_hdf5_hnsw_baseline.rs`、`benchmark_results/hnsw_search_cost_diagnosis.json`。
  2. 阶段结论：Task 10 已经说明继续围绕 generic/bitset kernel 做小改动，如果没有专门的 measurement lane，就很容易落入“代码收敛了，但当前 HDF5 lane 主要还是在看 layer-0 L2 fast path”的盲区。因此本轮不再继续改搜索核，而是先把 dedicated bitset diagnosis surface 补齐。
  3. 本轮执行：新增 `tests/bench_hnsw_search_first_bitset_diagnosis.rs`，先用缺失 `benchmark_results/hnsw_bitset_search_cost_diagnosis.json` 的失败做 TDD red；随后在 `src/faiss/hnsw.rs` 中增加 `search_single_candidate_profiled_with_bitset(...)` 与 `search_cost_diagnosis_with_bitset(...)`，再把 `src/bin/generate_hdf5_hnsw_baseline.rs` 扩成支持 `--bitset-diagnosis-output` / `--bitset-step`、filtered exact ground truth、repeated `search_with_bitset(...)` replay，并最终生成新的 local artifact `benchmark_results/hnsw_bitset_search_cost_diagnosis.json`。
  4. 验证结果：`cargo test --test bench_hnsw_search_first_bitset_diagnosis -- --nocapture` 先因缺失 artifact 失败后转绿；`cargo test test_search_cost_diagnosis_with_bitset_reports_visited_nodes --lib -- --nocapture` 先因缺失 API 失败后转绿；release HDF5 command 成功写出 canonical artifact 与 `/tmp` rerun artifact；canonical artifact 固定 `bitset_step=3`、`filtered_fraction=0.33334`、`build_random_seed=42`、`qps_repeat_count=3`，并在 `ef=64/96/128/160` 上给出约 `3729/3710/3751/3755` qps、固定 recall `0.9942`，同时 visited/distance counts 随 `ef` 稳定增长；same-code rerun 中 recall / visited / distance-calls 全部逐项完全一致，median-qps drift 约为 `-0.47% .. +1.01%`。
  5. 后续主缺口：这轮不是性能 gain，而是把 generic/bitset path 的 local measurement spine 补齐了；因此本轮可以记为 `screen_result=promote`。下一步应该停止继续做 harness 改造，转回真正的 generic/bitset kernel slice，并直接用 `benchmark_results/hnsw_bitset_search_cost_diagnosis.json` 作为本地 promote / reject 基线。

- 2026-03-14: **builder-loop：执行 `hnsw-search-first-task10` 本地实现 slice，把 generic/L2 heap frontier-results 真正收进 `SearchScratch`（screen/impl）**
  1. 复核输入：`task-progress.md`、`docs/superpowers/plans/2026-03-14-hnsw-search-first-task10.md`、`tests/bench_hnsw_generic_search_kernel.rs`、`benchmark_results/hnsw_search_cost_diagnosis.json`、`src/faiss/hnsw.rs`。
  2. 阶段结论：Task 9 只是把 generic unfiltered 和 bitset traversal 代码合并到同一个 helper，但 helper 内部还在每次 layer-search 新建两个 `BinaryHeap`。这和 `search_first` 想做的“便宜化通用搜索核”不一致，所以本轮继续做一刀更小的内核收敛：把 generic frontier/results 也收进 `SearchScratch`，同时覆盖 upper-layer L2 heap path，先把重复分配拿掉再看本地 seeded lane 有没有信号。
  3. 本轮执行：扩展 `tests/bench_hnsw_generic_search_kernel.rs`，先用缺失 `frontier_reuse_scope` / `result_reuse_scope` 的失败做 TDD red；随后在 `src/faiss/hnsw.rs` 中把 generic frontier/result `BinaryHeap` 提升为 `SearchScratch` 成员，新增 `prepare_generic_heaps(...)` 与 `drain_generic_results_sorted()`，并把 `search_layer_idx_shared(...)` 和 `search_layer_idx_l2_heap_with_optional_profile(...)` 都改成复用 scratch-owned heaps；同时补 focused library tests，锁定共享 generic kernel 与 upper-layer L2 heap kernel 在重复调用时都保持结果不变且容量可复用。
  4. 验证结果：`cargo test --test bench_hnsw_generic_search_kernel -- --nocapture` 先因缺字段失败后转绿；`cargo test test_search_layer_idx_shared_reuses_generic_heap_capacity_across_calls --lib -- --nocapture` 与 `cargo test test_search_layer_idx_l2_heap_reuses_generic_heap_capacity_across_calls --lib -- --nocapture` 都先因 `SearchScratch` 缺失 generic heaps 失败后转绿；`cargo test hnsw --lib -- --nocapture`、`cargo fmt --all -- --check`、`python3 scripts/validate_features.py feature-list.json` 最终通过；本地 seeded repeat-3 HDF5 lane重跑后，相比前一版 canonical baseline 的 median-qps 只变化 `-0.81% .. +1.42%`，再做一次同代码 `/tmp` rerun 后，same-code drift 为 `-0.27% .. +1.01%`，而 recall / visited / distance-calls 全部逐项完全一致。
  5. 后续主缺口：这说明 scratch-backed heap reuse 对代码结构是正向收敛，但在当前 local diagnosis lane 上还没有超出噪声带的吞吐信号。下一步如果继续 `search_first`，要么直接打到当前 seeded lane 真正在跑的路径，要么补一条专门针对 generic/bitset kernel 的 diagnosis surface；否则很容易继续做出“实现更好但 benchmark 看不见”的 slice。

- 2026-03-14: **builder-loop：执行 `hnsw-search-first-task9` 本地实现 slice，把 generic/bitset layer-search 真正收敛到一个共享 traversal core（screen/impl）**
  1. 复核输入：`task-progress.md`、`docs/superpowers/plans/2026-03-14-hnsw-search-first-task9.md`、`docs/superpowers/plans/2026-03-13-hnsw-unified-core-rewrite-roadmap.md`、`benchmark_results/hnsw_core_rewrite_decision_gate.json`、`src/faiss/hnsw.rs`。
  2. 阶段结论：Stage-1 已经把下一条 pure-Rust HNSW 主线收敛成 `search_first`，所以本轮不再做新的 graph/build 猜想，也不直接重开 authority lane；先做一刀最小代码收敛，把当前仍然近似重复的 generic unfiltered traversal 和 bitset traversal 真正归到一个 `idx`-based shared kernel 上，再决定是否值得重跑本地 search-cost diagnosis。
  3. 本轮执行：新增 `tests/bench_hnsw_generic_search_kernel.rs`，先用缺失 `generic_search_kernel_report()` 的失败做 TDD red；随后在 `src/faiss/hnsw.rs` 增加 `HnswGenericSearchKernelReport` 和 `generic_search_kernel_report()`，并把 `search_layer_idx_with_optional_profile()` 与 `search_layer_idx_with_bitset_scratch()` 都改成走新的 `search_layer_idx_shared(...)`；同时补 focused library tests，锁定共享 helper 对 unfiltered 和 bitset layer-0 结果都保持语义不变。
  4. 验证结果：`cargo test --test bench_hnsw_generic_search_kernel -- --nocapture` 先因缺失 `generic_search_kernel_report()` 失败后转绿；`cargo test test_shared_layer_search_matches_generic_layer0_results --lib -- --nocapture` 与 `cargo test test_shared_layer_search_matches_bitset_layer0_results --lib -- --nocapture` 通过；`cargo test hnsw --lib -- --nocapture` 通过；`cargo fmt --all -- --check` 初次因格式差异失败，`cargo fmt --all` 后转绿。
  5. 后续主缺口：在第一次 post-refactor rerun 里，builder-loop 发现这条 local diagnosis lane 还混着 build drift，因为 HNSW 之前无视了现成的 `config.params.random_seed`。本轮已经把 `random_seed` 接进 `src/faiss/hnsw.rs`，再给 `src/bin/generate_hdf5_hnsw_baseline.rs` 加了 `--random-seed` 与 `--repeat`；same-seed repeat-3 rerun 证明 recall / visited / distance-calls 现在可以稳定复现，而 median-qps 漂移也已经压到大约 1% 量级。因此下一步可以停止继续修 harness，转回真正的 pure-Rust `search_first` kernel work，并用这条 seeded repeat-3 lane 作为未来 local promote / reject 的可信基线。

- 2026-03-13: **builder-loop：执行 `hnsw-unified-core-rewrite-stage1` 本地 screen，把 pure-Rust HNSW 下一条主线从 reopen 猜想收敛成 branch decision（screen）**
  1. 复核输入：`task-progress.md`、`docs/superpowers/specs/2026-03-13-hnsw-unified-core-rewrite-roadmap-design.md`、`docs/superpowers/plans/2026-03-13-hnsw-unified-core-rewrite-roadmap.md`、`src/faiss/hnsw.rs`、`src/bin/generate_hdf5_hnsw_baseline.rs`。
  2. 阶段结论：round10/round11 已经说明继续围绕单个 locality 或 filtered-policy reopen 做 authority 试探的性价比很差，下一步应该先在本地 screen 上回答“是先重写 graph/build，还是先重写 generic search kernel”。这个问题不需要先开新的 tracked feature，但需要一条最小、可重复的 local diagnosis spine。
  3. 本轮执行：新增 `tests/bench_hnsw_core_rewrite_stage1.rs`，并先用缺失 `benchmark_results/hnsw_core_rewrite_baseline_lock.json`、`benchmark_results/hnsw_graph_diagnosis_rust.json`、`benchmark_results/hnsw_search_cost_diagnosis.json`、`benchmark_results/hnsw_core_rewrite_decision_gate.json` 的失败做 TDD red；随后在 `src/faiss/hnsw.rs` 增加 `graph_diagnosis_report()` 与 `search_cost_diagnosis()` 两个可复用诊断面，新增 `src/bin/hnsw_graph_diagnose.rs`，并把 `src/bin/generate_hdf5_hnsw_baseline.rs` 扩成支持 `--diagnosis-output` / `--ef-sweep` 的本地诊断模式；最终生成了 Stage-1 baseline lock、Rust graph artifact、search-cost artifact 和 decision gate artifact。
  4. 验证结果：本地 `cargo test --test bench_hnsw_core_rewrite_stage1 -- --nocapture` 先红后绿；`cargo test test_graph_diagnosis_report_counts_nodes --lib -- --nocapture`、`cargo test test_search_cost_diagnosis_reports_visited_nodes --lib -- --nocapture` 先因缺失 API 失败后转绿；`cargo run --release --bin hnsw_graph_diagnose -- --output benchmark_results/hnsw_graph_diagnosis_rust.json` 成功；`cargo run --release --features hdf5 --bin generate_hdf5_hnsw_baseline -- --input data/sift/sift-128-euclidean.hdf5 --output benchmark_results/rs_hnsw_sift128.local.json --diagnosis-output benchmark_results/hnsw_search_cost_diagnosis.json --ef-sweep 64,96,128,160 --top-k 100 --recall-at 10 --m 16 --ef-construction 100 --ef-search 138 --recall-gate 0.95` 成功；`cargo test hnsw --lib -- --nocapture` 与 `cargo fmt --all -- --check` 最终均通过。
  5. 后续主缺口：当前 screen 已经足够支撑 `search_first` 的下一阶段计划，但这仍然只是 local screen evidence，不是 authority verdict。下一条真正的 tracked/authority-grade HNSW work 应该围绕 unified generic search kernel 收敛展开，而不是再开一个 layer-0 或 filtered-policy 微假设 reopen。
...
- 2026-03-13: **builder-loop：收口 `hnsw-layer0-slab-audit-round10` 与 `hnsw-round10-authority-same-schema-rerun`，给 round10 locality hypothesis 做 authority 终判（plan+exec）**
  ...
  3. 本轮执行：先完成 `tests/bench_hnsw_reopen_round10_profile.rs` 与 `benchmark_results/hnsw_reopen_layer0_slab_audit_round10.json`，把 `layer0_slab` production layout、`flat_graph_profiled` fallback layout、`stride=644` 与 `vector_offset=132` 锁成 durable evidence；随后重跑 authority Rust HDF5 same-schema command 与 fresh native HNSW capture，把当前结果写回 `benchmark_results/rs_hnsw_sift128.full_k100.json` 和 `benchmark_results/hnsw_reopen_round10_authority_summary.json`，最终把 round 10 结论定为“hard stop on layer0_slab_locality”。
  4. 验证结果：本地 `cargo test hnsw --lib -- --nocapture`、`cargo test --features long-tests --test bench_hnsw_reopen_round10_profile -- --ignored --nocapture`、`cargo test --test bench_hnsw_reopen_round10 -- --nocapture`、`cargo fmt --all -- --check` 均通过；authority Rust same-schema HDF5 日志 `/data/work/knowhere-rs-logs-hnsw-reopen-round10/test_20260313T061915Z_87617.log` 通过，得到 `1831.504` qps / `0.9957` recall；fresh native capture 日志 `/data/work/knowhere-rs-logs-hnsw-reopen-round10/native_hnsw_qps_linkfix_20260313T061934Z.log` 通过，得到 `8900.882` qps / `0.95` recall；authority profile replay 日志 `/data/work/knowhere-rs-logs-hnsw-reopen-round10/test_20260313T063647Z_92534.log` 通过；authority default-lane replay 日志 `/data/work/knowhere-rs-logs-hnsw-reopen-round10/test_20260313T063939Z_93275.log` 通过。
  5. 后续主缺口：当前不再有排队中的 tracked feature。round 10 证明 slab-backed layer-0 layout 可以稳定接入 production fast path，但没有带来可归因的 Rust-side same-schema qps 提升，所以这条 locality hypothesis 应按 `hard_stop` 归档，不应继续默认延长。
