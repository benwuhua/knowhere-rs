...
- x86 prefetch ISA screen status (2026-03-16): added runtime-selectable hints (`t0|t1|t2|nta`) and observed directional small-sample signal (`t2` > `t1` > `nta` ~ `t0`) on authority, but variance is high; require full-size authority `t0` vs `t2` confirmation before changing the default hint.
