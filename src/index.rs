...
pub trait BinaryIndex: Send + Sync {
    ...
    /// 训练索引
    fn train(&mut self, dataset: &crate::dataset::BinaryDataset) -> Result<(), IndexError>;
...
pub trait Index: Send + Sync {
    ...
    fn range_search(&self, _query: &Dataset, _radius: f32) -> Result<SearchResult, IndexError> {
        ...
        Err(IndexError::Unsupported(
            "range_search not implemented".into(),
        ))
    }
    ...
    fn get_vector_by_ids(&self, _ids: &[i64]) -> Result<Vec<f32>, IndexError> {
        Err(IndexError::Unsupported(
            "get_vector_by_ids not implemented".into(),
        ))
    }
    ...
    fn serialize_to_memory(&self) -> Result<Vec<u8>, IndexError> {
        Err(IndexError::Unsupported(
            "serialize_to_memory not implemented".into(),
        ))
    }
...
mod tests {
    ...
    fn test_search_result() {
        let result = SearchResult::new(vec![1, 2, 3], vec![0.1, 0.2, 0.3], 1.5);
