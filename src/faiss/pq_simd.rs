...
impl PqDistance {
    ...
    fn distance_scalar(&self, query: &[f32], codebook: &[f32], codes: &[u8]) -> f32 {
        ...
        for (sub_idx, &codeword) in codes.iter().enumerate().take(self.m) {
    ...
    /// 批量距离计算
    pub fn distance_batch(&self, query: &[f32], codebook: &[f32], codes: &[Vec<u8>]) -> Vec<f32> {
        codes
            .iter()
...
impl DistanceTable {
    ...
    pub fn query_distance(&self, codes: &[u8]) -> f32 {
        let mut sum = 0.0f32;
        for (m_idx, &codeword) in codes.iter().enumerate().take(self.m) {
            let code = codeword as usize;
