...
impl SparseInvertedIndex {
    ...
    fn parse_query(&self, query: &SparseVector, drop_ratio: f32) -> Vec<(u32, f32)> {
        if drop_ratio == 0.0 {
            ...
            query
                .elements
                .iter()
                .filter_map(|e| self.dim_map.get(&e.dim).map(|&inner_id| (inner_id, e.val)))
                .collect()
        } else {
            // 计算阈值
            let mut abs_vals: Vec<f32> = query.elements.iter().map(|e| e.val.abs()).collect();
    ...
    pub fn get_posting_doc(&self, term_idx: usize, cursor: usize) -> Option<i64> {
        self.inverted_index_ids
            .get(term_idx)
            .and_then(|list| list.get(cursor).copied())
    }
...
mod tests {
    ...
    fn test_index_size() {
        ...
        for i in 0..100i64 {
            let vec =
                SparseVector::from_pairs(&[(i as u32 % 10, 1.0), (((i + 5) % 10) as u32, 2.0)]);
    ...
    fn test_wand_search_basic() {
        ...
        index
            .add(
                &SparseVector::from_pairs(&[(0, 1.0), (1, 1.0), (2, 1.0)]),
                3,
            )
            .unwrap();
