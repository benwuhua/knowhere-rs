...
impl SparseVector {
    ...
    /// 计算向量元素绝对值的和 (用于 BM25)
    pub fn sum(&self) -> f32 {
        self.elements.iter().map(|e| e.val.abs()).sum()
    }
...
impl SparseInvertedIndexCC {
    ...
    pub fn train(&self, vectors: &[SparseVector]) -> Result<()> {
        let mut inner = self
            .inner
            .write()
            .map_err(|e| KnowhereError::InternalError(format!("RwLock poisoned: {}", e)))?;
    ...
    pub fn add(&self, vector: &SparseVector, doc_id: i64) -> Result<()> {
        ...
        // 更新 n_dims (取最大维度索引 + 1)
        if let Some(max_dim) = vector.elements.iter().map(|e| e.dim).max() {
            inner.n_dims = inner.n_dims.max((max_dim + 1) as usize);
        }
    ...
    pub fn get_vectors_by_ids(&self, ids: &[i64]) -> Vec<Option<SparseVector>> {
        ...
        ids.iter()
            .map(|&id| inner.as_ref().and_then(|g| g.doc_vectors.get(&id).cloned()))
            .collect()
    }
...
mod tests {
    ...
    fn test_sparse_inverted_index_cc_search() {
        ...
        // 添加向量
        index
            .add(&SparseVector::from_pairs(&[(0, 1.0), (1, 1.0)]), 0)
            .unwrap();
        index
            .add(&SparseVector::from_pairs(&[(1, 1.0), (2, 1.0)]), 1)
            .unwrap();
        index
            .add(&SparseVector::from_pairs(&[(0, 1.0), (2, 1.0)]), 2)
            .unwrap();
        ...
        // 所有文档的 IP 分数都是 2.0 (1+1)，顺序可能不确定
        // 验证返回了 2 个有效结果
        assert_eq!(results.len(), 2);
        for (doc_id, score) in &results {
            assert!(*doc_id >= 0 && *doc_id <= 2);
    ...
    fn test_sparse_inverted_index_cc_bm25() {
        ...
        index
            .add(
                &SparseVector::from_pairs(&[(0, 1.0), (1, 2.0), (2, 1.0)]),
                0,
            )
            .unwrap();
        index
            .add(&SparseVector::from_pairs(&[(0, 1.0), (3, 3.0)]), 1)
            .unwrap();
        index
            .add(
                &SparseVector::from_pairs(&[(1, 1.0), (2, 2.0), (3, 1.0)]),
                2,
            )
            .unwrap();
