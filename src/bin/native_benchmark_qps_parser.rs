...
fn parse_native_benchmark_log(text: &str, requested_thread_num: Option<u32>) -> Result<NativeBenchmarkRow, String> {
    ...
    if let Some(thread_num) = requested_thread_num {
        return candidates
            .into_iter()
            .find(|row| row.thread_num == thread_num)
...
fn print_usage(program: &str) {
    eprintln!(
        "Usage: {program} --input <path> [--thread-num <n>]\n\
...
fn main() {
    ...
    match parse_native_benchmark_log(&text, thread_num) {
...
"#;
    ...
    fn parses_highest_thread_row_by_default() {
        let row = parse_native_benchmark_log(SAMPLE, None).expect("parse sample");
    ...
    fn parses_requested_thread_row() {
        let row = parse_native_benchmark_log(SAMPLE, Some(1)).expect("parse sample thread 1");
    ...
    fn rejects_missing_thread() {
        let err = parse_native_benchmark_log(SAMPLE, Some(16)).expect_err("missing thread should fail");
