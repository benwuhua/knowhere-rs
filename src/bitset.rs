...
impl BitsetView {
    ...
    pub fn set_out_ids(&mut self, out_ids: Vec<u32>, num_filtered_out_ids: Option<usize>) {
        ...
        let filtered_count = num_filtered_out_ids.unwrap_or_else(|| {
            ...
            // 需要手动进行 ID 映射并检查位
            let mut count = 0;
            for &external_id_u32 in out_ids.iter().take(num_internal_ids) {
                let external_id = external_id_u32 as usize;
                // 检查外部 ID 是否被过滤（位为 1）
...
#[cfg(target_arch = "aarch64")]
/// # Safety
/// `start_bit / 8` must point to a valid readable region of at least 16 bytes in `data`
/// when the SIMD path is taken.
...
pub fn test_batch_fallback(data: &[u8], start_bit: usize) -> bool {
    ...
    for &byte in data.iter().take(end_byte).skip(start_byte) {
        if byte != 0 {
            return false;
        }
    }
...
#[cfg(target_arch = "aarch64")]
/// # Safety
/// `data.as_ptr()` must remain valid for 16-byte vector loads while this function executes.
...
pub fn test_batch_auto(data: &[u8], start_bit: usize) -> bool {
    #[cfg(target_arch = "x86_64")]
    {
        if is_x86_feature_detected!("avx2") {
            unsafe {
                return test_batch_avx2(data, start_bit);
            }
        }
    }
    ...
    #[cfg(target_arch = "aarch64")]
    {
        if is_aarch64_feature_detected!("neon") {
            unsafe {
                return test_batch_neon(data, start_bit);
            }
        }
    }
...
pub fn count_zero_batch_auto(data: &[u8]) -> usize {
    #[cfg(target_arch = "x86_64")]
    {
        if is_x86_feature_detected!("avx2") {
            unsafe {
                return count_zero_batch_avx2(data);
            }
        }
    }
    ...
    #[cfg(target_arch = "aarch64")]
    {
        if is_aarch64_feature_detected!("neon") {
            unsafe {
                return count_zero_batch_neon(data);
            }
        }
    }
...
#[inline]
pub fn test_batch(data: &[u64], _start_bit: usize, _num_bits: usize) -> bool {
...
#[inline]
pub fn count_zero_batch(data: &[u64], _num_bits: usize) -> usize {
    let byte_slice = bytemuck::cast_slice::<u64, u8>(data);
...
#[inline]
pub fn test_batch_generic(data: &[u64], _start_bit: usize, _num_bits: usize) -> bool {
    let byte_slice = bytemuck::cast_slice::<u64, u8>(data);
...
#[inline]
pub fn count_zero_batch_generic(data: &[u64], _num_bits: usize) -> usize {
    let byte_slice = bytemuck::cast_slice::<u64, u8>(data);
...
mod tests {
    ...
    unsafe fn test_neon_intrinsics_impl() {
        ...
        assert!(
            min_val == 0xFF,
            "All-zero data should compare equal to zero"
        );
        ...
        assert!(
            min_val == 0,
            "All-ones data should not compare equal to zero"
        );
    ...
    fn benchmark_simd_performance() {
        ...
        println!(
            "  Fallback: {:?} ({:.2} ns/iter)",
            fallback_time,
            fallback_time.as_nanos() as f64 / iterations as f64
        );
        println!(
            "  Auto:     {:?} ({:.2} ns/iter)",
            auto_time,
            auto_time.as_nanos() as f64 / iterations as f64
        );
    ...
    unsafe fn benchmark_avx2_specific_impl() {
        ...
        println!(
            "AVX2 count_zero: {:?} ({:.2} ns/iter)",
            avx2_count_time,
            avx2_count_time.as_nanos() as f64 / iterations as f64
        );
    }
    ...
    unsafe fn benchmark_neon_specific_impl() {
        ...
        println!(
            "NEON test_batch: {:?} ({:.2} ns/iter)",
            neon_time,
            neon_time.as_nanos() as f64 / iterations as f64
        );
