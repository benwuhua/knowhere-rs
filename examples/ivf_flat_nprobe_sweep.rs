...
const OUTPUT_JSON: &str = "/tmp/ivf_flat_nprobe_result.json";
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut ivf_index = IvfFlatIndex::new(&ivf_cfg)?;
    ivf_index.train(&base)?;
    ivf_index.add(&base, None)?;
    ...
    for &nprobe in &NPROBE_SWEEP {
        ...
        for (i, gt_ids) in gt_top10.iter().enumerate() {
            let q = &recall_queries[i * DIM..(i + 1) * DIM];
            let res = ivf_index.search(q, &req)?;
        ...
        println!(
            "nprobe={:>3} recall@10={:.3} qps={}",
            nprobe, recall_rounded, qps
        );
    ...
    let mut min_gate: Option<&SweepResult> = None;
    for row in &rows {
    ...
    let status_line = if let Some(best) = min_gate {
        format!(
            "DONE: ivf-flat nprobe sweep done. min nprobe for recall≥0.95: {} (qps={}). full results /tmp/ivf_flat_nprobe_result.json\n",
            best.nprobe, best.qps
        )
    } else {
        "DONE: ivf-flat nprobe sweep done. min nprobe for recall≥0.95: N/A (qps=N/A). full results /tmp/ivf_flat_nprobe_result.json\n".to_string()
    };
