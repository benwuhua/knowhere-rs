...
fn bench_rabitq_recall(c: &mut Criterion) {
    let mut group = c.benchmark_group("rabitq_recall");
    ...
    println!("\n=== RaBitQ 召回率基准测试 ===");
    println!("数据集：{} 向量，{} 维", n_database, dim);
    println!("查询数：{}", n_query);
    println!("压缩比：{:.2}x", compression_ratio);
    println!("原始大小：{} MB", raw_size / 1024 / 1024);
    println!("RaBitQ 大小：{} MB\n", rabitq_size / 1024 / 1024);
    ...
    for nprobe in nprobe_values {
        ...
        for i in 0..n_query {
            ...
            let r10 = compute_recall(query, &rabitq_results.ids, &gt_results.ids, 10);
            let r100 = compute_recall(query, &rabitq_results.ids, &gt_results.ids, 100);
        ...
        println!(
            "nprobe={}: R@10={:.4}, R@100={:.4}",
            nprobe, avg_r10, avg_r100
        );

        group.bench_function(BenchmarkId::new("r10", nprobe), |b| {
            b.iter(|| {
                let query = &queries[0..dim];
                ...
                black_box(rabitq_index.search(black_box(query), &req).unwrap())
            })
        });
    }
